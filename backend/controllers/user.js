...
const createApplication = async (req, res) => {
	try {
		...
		res
			.status(500)
			.json({ error: "Something went wrong while creating the application" });
	}
};
	...
	try {
		...
		if (req.files) {
			if (req.files.linkedin_screenshot) {
				...
				req.body.linkedin_post_screenshot_url = await uploadToS3(
					req.files.linkedin_screenshot[0]
				);
			}
			if (req.files.payment_screenshot) {
				...
				req.body.payment_screenshot_url = await uploadToS3(
